...
export default function Generate() {
  ...
  return (
    <div className="page-root" style={{ minHeight: "100vh", padding: "24px", paddingBottom: "60px" }}>
      <div style={{ maxWidth: "680px", margin: "0 auto", paddingTop: "96px" }}>
        {/* Header */}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="fade-in">
          <div style={{ display: "flex", flexDirection: "column", gap: "16px", marginBottom: "20px" }}>
            {fields.map((field, index) => (
              <div key={field.id} className="card-sm" style={{ padding: "22px 24px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: "12px" }}>
                  <div className="field-group">
                    ...
                    />
                    {errors.links?.[index]?.platform && <p className="error-text">{errors.links[index]?.platform?.message}</p>}
                  </div>
                  <div className="field-group">
                    ...
                    />
                    {errors.links?.[index]?.accountLink && <p className="error-text">{errors.links[index]?.accountLink?.message}</p>}
                  </div>
                </div>
              </div>
            ))}
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "12px" }}>
            ...
            >
              {isSubmitting ? (
                <><span className="spinner" style={{ border: "3px solid rgba(255,255,255,0.3)", borderTopColor: "#fff", width: "16px", height: "16px" }}></span> Saving…</>
              ) : "Save Links"}
            </button>
          </div>
        </form>
